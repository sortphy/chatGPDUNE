      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="relative bg-[#2b1b11]/60 backdrop-blur-sm rounded-2xl border border-[#3f2e1e]/50 shadow-2xl transition-all duration-200 focus-within:border-[#8b4513]/60 focus-within:shadow-[#8b4513]/10">
          <textarea
            ...
            className="w-full bg-transparent text-white placeholder-[#a67c52] px-6 py-4 pr-24 resize-none focus:outline-none min-h-[60px] max-h-32"
            disabled={isLoading}
            rows={1}
            style={{ 
              resize: 'none',
          ...
          <button
            ...
            disabled={isLoading}
            className={`absolute right-16 bottom-3 w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 ${
              useRag 
                ? 'bg-gradient-to-r from-[#8b4513] to-[#cd853f] hover:shadow-xl' 
                : 'bg-[#3f2e1e]/60 hover:bg-[#3f2e1e]/80'
            }`}
            title={`RAG ${useRag ? 'ON' : 'OFF'} - ${useRag ? 'Using knowledge base' : 'Using AI only'}`}
          >
        ...
        {/* Footer with RAG Status */}
        <div className="text-center text-xs text-[#a67c52] mt-3">
          Using Ollama & DeepSeek R1 • RAG {useRag ? 'ON' : 'OFF'} • Às vezes demora, depende da máquina, pois roda local.
        </div>
      </div>
    </div>
  );
